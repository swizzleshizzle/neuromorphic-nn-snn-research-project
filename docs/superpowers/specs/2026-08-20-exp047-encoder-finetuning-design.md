...
unmodified (**~1.6 h at 12 workers**, EXP-040's own measured figure), which is a prerequisite of
the fallback experiment anyway.
...
measured at the exposure it governs. Cost: 6 cells, **~5.6 h** (see section 8).
...
|---|---|---|---|---|
| 0 | EXP-040 `pretrain_one`, seeds 12-23 | - | 12 | **~1.6 h** |
...
> **The ~1.6 h for phase 0 is EXP-040's OWN measured figure**, from its `RESULTS.md`
> ("Phase 1 pretrains 12 encoders (~1.6 h at 10 workers)"). The 2026-08-20 handoff carried
> **~20 min**, derived by dividing EXP-039's ~900-1000 s per seed by 12 workers. That arithmetic
> assumes perfect 12-way parallelism; the laptop delivers about **6.6 effective cores** at 12
> workers, and the per-seed cost is higher than the VPS figure besides. It is roughly **5x
> optimistic**, and it was inherited into this spec before being checked against the experiment
> that actually ran the operation. Confirmed live: 81 minutes in, phase 0 was consuming 6.56
> effective cores and had written nothing yet, which is on track rather than stalled.
