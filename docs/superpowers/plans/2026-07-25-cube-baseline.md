...
**Why this shape:** `action_distribution` in `reinforce.py` reads only `brain.content` and `out["concept"]`, and `make_policy_head` reads `brain.content` and `brain.n_actions`. In v1 the action comes from the policy head, not the brain's motor readout (ADR-0001 Amendment 1), so the control legitimately has no motor. With `recall=False` and the policy head reading the sensory concept, the regionalized arm's policy path is the sensory region only (144 -> 128 hidden -> 64 concept): hippocampus (150 neurons) never executes, and prefrontal (150), router (12) and motor (6) run but their outputs are discarded. So matching **total** neurons means `Brain` spends 318 of its 510 neurons off the policy path, not 18. The monolithic arm has no such waste; it spends its whole 510-neuron budget on the path (144 -> 446 hidden -> 64 concept). Total-neuron matching is therefore not conservative in the control's favor: it is a 128-wide frozen stack on the policy path compared against a 446-wide one, an asymmetry the comparison must be read with, not a bias that simply favors the control.
