...
Extend `trainable_params` to include `encoder_optimizer`'s groups when present, so a gated run still reports 27,206 rather than 390:
...
def test_arm_b_is_exp051_plus_a_critic(tmp_path):
    ...
    for c in cfgs:
        ...
        assert c.critic_lr == m.SELECTED_CRITIC_LR
