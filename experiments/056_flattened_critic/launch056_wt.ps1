# EXP-056 launcher for SwizzlesDuo, running from the WORKTREE at C:\Users\mlgbr\wt-exp053.
#
#   powershell -NoProfile -ExecutionPolicy Bypass -File C:\Users\mlgbr\launch056_wt.ps1 -Phase check
...
$hasFlag = & $py -c "from neuromorphic.training.cube_baseline import CubeConfig; import sys; sys.stdout.write(str(hasattr(CubeConfig, '__dataclass_fields__') and 'flatten_critic' in CubeConfig.__dataclass_fields__))"
if ($hasFlag -ne "True") {
    Write-Error "CubeConfig has no flatten_critic field. The worktree is stale; this would silently re-run arm B."
    exit 1
}
...
"flatten_critic present = $hasFlag"
...
$outDir = Join-Path $wt "experiments\056_flattened_critic\outputs"
...
$script  = "experiments\056_flattened_critic\run.py"
$cliArgs = @("--workers", $Workers)
if ($SkipExisting) { $cliArgs += "--skip-existing" }
$log = Join-Path $wt "experiments\056_flattened_critic\phase_rl.log"
