...
def test_arm_b_is_exp051_plus_a_critic(tmp_path):
    m = _module()
    # critic_lr supplied explicitly: the real value comes from `select_critic_lr.py`'s output
    # on disk (`read_selected_critic_lr`), which this tmp_path deliberately does not have.
    cfgs = m.sweep_configs("B", (0, 1), tmp_path, critic_lr=1e-2)
    assert len(cfgs) == 2
    for c in cfgs:
        ...
        assert c.critic_lr == 1e-2, f"the supplied critic_lr did not arrive on the config: {c.critic_lr!r}"
        # 047, not 040: arm B loads E1 (EXP-047's fine-tuned encoder), unlike arms G and R
        # which load E0 (EXP-040's pretrained encoder, their own control's starting point).
...
def test_arm_g_is_exp047_plus_a_gate(tmp_path):
    ...
    for c in m.sweep_configs("G", (0, 1), tmp_path):
        ...
        assert "040_pretrained_encoder_policy" in str(c.encoder_state_path), (
            "arm G must load EXP-047's OWN starting encoders (EXP-040's), or it differs from "
            "its control in the encoder as well as the gate"
        )
...
def test_record_filenames_do_not_collide(tmp_path):
    ...
    m = _module()
    names = []
    for arm, kw in (("B", {"critic_lr": 1e-2}), ("G", {}),
                    ("R", {"rates": {s: 0.5 for s in range(12)}})):
